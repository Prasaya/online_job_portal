...
export const deleteJobPost = async (jobId: string) => {
  try {
    await connection.execute('DELETE FROM jobs ' + 'WHERE jobId = ?', [jobId]);
    await connection.execute('DELETE FROM skills ' + 'WHERE jobId = ?', [
      jobId,
    ]);
    await connection.execute(
      'DELETE FROM job_qualifications ' + 'WHERE jobId = ?',
      [jobId],
    );
  } catch (error) {
